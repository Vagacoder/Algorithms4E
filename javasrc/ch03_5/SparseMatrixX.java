...
public class SparseMatrixX {
    ...
    private SparseVectorX[] rows;   // the rows, each row is a sparse vector

    // initialize an n-by-n matrix of all 0s
    public SparseMatrixX(int n) {
        this.n = n;
        rows = new SparseVectorX[n];
        for (int i = 0; i < n; i++)
            rows[i] = new SparseVectorX(n);
    }
    ...
    public SparseVectorX times(SparseVectorX x) {
        if (n != x.size()) throw new IllegalArgumentException("Dimensions disagree");
        SparseVectorX b = new SparseVectorX(n);
        for (int i = 0; i < n; i++)
    ...
    // return this + that
    public SparseMatrixX plus(SparseMatrixX that) {
        if (this.n != that.n) throw new RuntimeException("Dimensions disagree");
        SparseMatrixX result = new SparseMatrixX(n);
        for (int i = 0; i < n; i++)
    ...
    public static void main(String[] args) {
        SparseMatrixX A = new SparseMatrixX(5);
        SparseVectorX x = new SparseVectorX(5);
        A.put(0, 0, 1.0);
        A.put(1, 1, 1.0);
        A.put(2, 2, 1.0);
        A.put(3, 3, 1.0);
